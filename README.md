...
The core is the vanilla API and needs no framework, and `@standarx/nav/auto` is a start-up helper next to it rather than an adapter ([ADR-0023](docs/adr/0023-vanilla-auto-mount.md)). React, Vue and Svelte are the adapters shipped today, all three held to the same parity suite ([ADR-0027](docs/adr/0027-vue-adapter.md) for Vue, [ADR-0028](docs/adr/0028-svelte-adapter.md) for Svelte); Angular follows ([ADR-0011](docs/adr/0011-package-layout-and-adapters.md)), held to that suite before it ships.

## Documentation

- [docs/en/](docs/en/): the attribute tables, how a move is decided and what makes an element navigable, React, Vue, Svelte, the auto-mount helper, the focus ring. Mirrored file by file in [docs/fr/](docs/fr/).
