...
[![core size](https://img.shields.io/badge/core-3.27%20kB%20min%2Bgzip-blue)](docs/adr/0017-size-budgets.md)
...
- Two modes: `composite` (arrow keys stay inside composites, a gamepad crosses the page) and `app` (arrow keys navigate the whole page). `onWillMove` can veto a move; a candidate the browser refuses to focus hands the move to the next one; `onBoundsHit` fires when a direction has nowhere left to go.
- Zero runtime dependencies, ESM only, and every subpath sits under a size cap CI enforces: 3.27 kB min+gzip for the core, 1.42 kB to add the React adapter next to it, 3.24 kB for the spatial engine, by `bun run check:size` on 2026-09-24 ([ADR-0017](docs/adr/0017-size-budgets.md)). You pay for what you import and nothing else.
