...
[![core size](https://img.shields.io/badge/core-3.38%20kB%20min%2Bgzip-blue)](docs/adr/0017-size-budgets.md)
...
**Status:** `0.3.0` is on npm since 2026-09-26. The public API is not frozen and the `data-snav-*` attribute names are not final ([ADR-0001](docs/adr/0001-name-scope-and-attribute-prefix.md)); while the major is 0, a minor may break. See [ROADMAP.md](ROADMAP.md).
...
- Two modes: `composite` (arrow keys stay inside composites, a gamepad crosses the page) and `app` (arrow keys navigate the whole page). `onWillMove` can veto a move; a candidate the browser refuses to focus hands the move to the next one, and the areas of an image map are candidates, each placed by its shape over its image ([ADR-0031](docs/adr/0031-image-map-area-candidate.md)); `onBoundsHit` fires when a direction has nowhere left to go.
- Zero runtime dependencies, ESM only, and every subpath sits under a size cap CI enforces: 3.38 kB min+gzip for the core, 1.42 kB to add the React adapter next to it, 3.54 kB for the spatial engine, by `bun run check:size` on 2026-09-26 ([ADR-0017](docs/adr/0017-size-budgets.md)). You pay for what you import and nothing else.
...
| `@standarx/nav/svelte` | `provideNav` and the functions around it, for Svelte 5.0 and later — [docs/en/svelte.md](docs/en/svelte.md). |
| `@standarx/nav/angular` | `provideNav` and the `inject*` functions, for Angular 20.0 and later — [docs/en/angular.md](docs/en/angular.md). |
...
The core is the vanilla API and needs no framework, and `@standarx/nav/auto` is a start-up helper next to it rather than an adapter ([ADR-0023](docs/adr/0023-vanilla-auto-mount.md)). React, Vue, Svelte and Angular are the adapters shipped today, which completes the order of [ADR-0011](docs/adr/0011-package-layout-and-adapters.md), all four held to the same parity suite ([ADR-0027](docs/adr/0027-vue-adapter.md) for Vue, [ADR-0028](docs/adr/0028-svelte-adapter.md) for Svelte, [ADR-0029](docs/adr/0029-angular-adapter.md) for Angular).

## Documentation

- [docs/en/](docs/en/): the attribute tables, how a move is decided and what makes an element navigable, React, Vue, Svelte, Angular, the auto-mount helper, the focus ring. Mirrored file by file in [docs/fr/](docs/fr/).
...
[CONTRIBUTING.md](CONTRIBUTING.md), in one line: conventional commits, `bun` never `npm`, a test with every behaviour change, blocking size budgets, English only, and no claim in a document without the command, date or URL that proves it. CI runs eleven checks on every pull request: lint, typecheck, build with the packaging and size gates, unit tests, a React 18.3 floor, a Vue 3.3.0 floor, a Svelte 5.0.0 floor, an Angular 20.0.0 floor, and the browser suite on chromium, firefox and webkit. Conduct is [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md); a vulnerability goes through [SECURITY.md](SECURITY.md), never a public issue.
