...
3. **Rename the attributes in the same branch.** `data-mira-nav*` →
   ...
   names three properties inside this repository's plugin — offset, duration and
   easing, the only three it reads (miralabs-ui:
   `packages/core/src/input/focus-ring/focus-ring.ts:84`, `:112`, `:115`). On
