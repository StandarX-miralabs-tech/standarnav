...
`react adapter`, `vue adapter`, `svelte adapter`, `keyboard` and one line per keyboard layout
(`scripts/size-budget.ts:80-179`), and `src/input-system.ts` is
...
(`.github/workflows/ci.yml:57-58`). It declares thirteen lines
(`scripts/size-budget.ts:80-179`), every one of them capped from a measurement
...
3. Caps are the measured value rounded **up to the next quarter kB**. That is
   ...
   (`scripts/size-budget.ts:386-390`).
...
5. **A line without a cap fails the run.** `cap: null` means "measured, not yet
   ...
   (`scripts/size-budget.ts:14-16`, `scripts/size-budget.ts:386-390`). A default
...
- No cap can be written before a build exists, so rule 5 keeps `check:size` red
  ...
  (`scripts/size-budget.ts:375-378`). The sequence is red, measure, cap, green, and
...
budget script makes a line without a cap fail the run (`scripts/size-budget.ts:386-390`). The
...
The split is what this repository does: thirteen separate lines, one per entry
(`scripts/size-budget.ts:80-179`).
...
here is already bundled through (`scripts/size-budget.ts:301-325`).
...
and the numbers are in `scripts/size-budget.ts:80-179`.
...
- This repository's `scripts/size-budget.ts`, declared as `check:size`
  (`package.json`): thirteen lines — `core` (`index.js`), `gamepad engine`, `spatial
  engine`, `focus ring`, `debug`, `auto mount`, `react adapter`, `vue adapter`, `svelte adapter`,
  `keyboard` and one per keyboard layout
  (`scripts/size-budget.ts:80-179`). Each opt-in line names the part of the core graph
  ...
  `scripts/size-budget.ts:87-159`), while `core` declares none. All thirteen caps are written
  ...
  `scripts/size-budget.ts:152`, `scripts/size-budget.ts:162`, `scripts/size-budget.ts:170`,
  `scripts/size-budget.ts:176`); a line whose `cap` is
  `null` prints its measurement
  and exits non-zero (`scripts/size-budget.ts:386-390`), and a missing `dist/` exits
  first with a message pointing at `bun run build`
  (`scripts/size-budget.ts:375-378`).
- The coverage check that replaced the whole-package line: `builtModules`
  (`scripts/size-budget.ts:188-200`) walks `dist/`, `chargedBy`
  (`scripts/size-budget.ts:223-237`) walks each line's graph and stops at its externals,
  and `uncoveredModules` (`scripts/size-budget.ts:244-246`) names what no line pays for.
  The graph is read off the built output by `moduleImports`
  (`scripts/size-budget.ts:208-216`) rather than asked of the bundler, because a catch-all
...
- Bun's default gzip level, and the instruction never to mix it with `gzip -9`:
  ...
  `scripts/size-budget.ts:5-7` and `scripts/size-budget.ts:301-325`.
...
- Measured here: `bun run build && bun run check:size` (both scripts in `package.json`) on
  ...
  adapter's, and on 2026-09-24 for the svelte adapter's; the lines and their
  caps are `scripts/size-budget.ts:80-179`, and the run is enforced in CI
