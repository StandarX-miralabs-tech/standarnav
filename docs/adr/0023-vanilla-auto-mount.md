...
the adapter order (`:128`), after React and before Vue, and leaves its subpath name the one
open detail of that record — working name `@standarx/nav/auto`, not confirmed
(`:7-8`). The same record is emphatic that the thing is not an adapter: "`vanilla` is not an
adapter, because the core is the vanilla API; the only vanilla-specific artefact is an
auto-mount helper reading `data-snav-*` attributes" (`:87-89`).
...
`/vanilla` would re-assert in a subpath name exactly what ADR-0011:87-88 denies.
...
`spatialPlugin`, `gamepadPlugin`, `focusRingPlugin` (ADR-0011:164-167) — and nothing runs until
it is called. The signature is `autoMount(options?: AutoMountOptions): AutoMount`
(`src/auto/auto.ts:65`), returning a handle carrying `system`, `null` until there is one, and an
idempotent `destroy` (`AutoMount` at `:54-59`).
...
(`src/auto/auto.ts:93-99`). A module script is deferred by the platform, so most callers never
...
(`MODE_ATTRIBUTE` at `src/auto/auto.ts:28`, `readMode` at `:61-63`). It is a **new** attribute:
...
(`:95`). That rejection stands untouched — see the amendment this record adds there.
...
(`src/auto/auto.ts:43`). An array alone could never be configured by the markup: a caller who
...
(`:30-37`). Plugins stay imports rather than attributes because an engine is code: a markup
...
re-exported (`:21`) because ADR-0011:68-73 requires a subpath to export the types its own
signatures name. The budget line measures 0.62 kB min+gzip against a 0.75 cap
...
[ADR-0018](0018-testing-strategy.md) decision 5 asserts eleven behaviours of a framework
provider, and its first one is "builds exactly one system, and not during the first render"
(`src/adapter-parity.ts:84`). There is no render here, and no provider: `ParityAdapter` would
...
- One more entry, one more subpath, one more budget line, and a whole-package cap that moved
  from 12.50 to 12.75 kB for 0.24 kB of it. The generated `exports` map gained `./auto`
...
| **`@standarx/nav/vanilla`** | ADR-0011:87-88 says "`vanilla` is not an adapter, because the core is the vanilla API". A subpath called `/vanilla` next to `/react` states the opposite in the one place a consumer reads first. |
...
| **`autoMount()` with no arguments, constructing `spatialPlugin` itself** | The most convenient form, and the one that breaks the layout: `/auto` would import `/spatial`, so ADR-0011's "an entry the consumer never imports is never bundled" (`:41-43`) would stop holding, and the line would go from 0.62 to roughly 3.4 kB. Rejected on that alone. |
| **No attribute at all, `mode` as an option** | Honest and smaller, and it leaves the helper with one behaviour — the `DOMContentLoaded` wait — which does not earn a public subpath. It would also make ADR-0011's "attribute-driven start-up" (`:128`) a description of nothing. |
| **Per-container `data-snav-mode`, the form ADR-0007 rejected** | Still rejected, for ADR-0007's own reason (`:95`): the mode would depend on where the focus is and could change mid-move. The attribute here is read once, on one element, before the engine exists. |
| **Running the adapter parity suite against it** | The suite's contract is a provider with a render pass and two nested scope components (`src/adapter-parity.ts:57-76`). Satisfying it would mean inventing that shape for an API that has none, which is a test asserting the fixture rather than the helper. |

## Evidence

- The rider this record closes, and the order it sits in:
  [ADR-0011](0011-package-layout-and-adapters.md) `:7-8` (the open subpath name), `:128` (second
  in the adapter order), `:87-89` (`vanilla` is not an adapter), `:41-43` (an entry the consumer
  never imports is never bundled), `:68-73` (a subpath re-exports the types its signatures name),
  `:164-167` (entries stay factory-based because `sideEffects: false` is a promise).
- The helper: `src/auto/auto.ts`, 116 lines — `MODE_ATTRIBUTE` (`:28`), `AutoConfig` (`:30-41`),
  `AutoPlugins` (`:43`), `AutoMountOptions` (`:45-52`), `AutoMount` (`:54-59`), `readMode`
  (`:61-63`), `autoMount` (`:65`), the still-parsing branch (`:93-99`).
...
- That `data-snav-mode` was new on this date: `MODALITY_ATTRIBUTE = "data-snav-input"`
  ...
  rejected per-container row (`0007-navigation-modes.md:95`), and mode until now was read from
...
- Size: `bun run build && bun run check:size` on 2026-09-22 — `auto mount` 1.04 kB min,
  0.62 kB min+gzip, cap 0.75 kB, 83 % used; `whole package` 12.64 of 12.75 kB.
  [ADR-0017](0017-size-budgets.md), amendment of 2026-09-22.
- Why the parity suite is not run here: its eleven cases and the `ParityAdapter` contract,
  `src/adapter-parity.ts:57-76` and `:84`; the suite's growth from six behaviours is recorded at
  [ADR-0018](0018-testing-strategy.md) `:145-153`.
