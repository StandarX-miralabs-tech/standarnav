...
| The spatial plugin's handling of `scrollX` | `scrollY` is covered by four cases (`src/spatial/spatial.browser.test.ts:348-398`); no case emits `scrollX` into the plugin |
...
|---|---|
| Scroll-and-rescan on a virtualised scroller | `src/spatial/spatial.browser.test.ts:728-786` — two cases: it scrolls when nothing is reachable and lands a frame later (`:755`), and it scrolls four fifths of the viewport rather than a whole one (`:775`) |
| `pointerFollowsFocus` in `app` mode | `src/spatial/spatial.browser.test.ts:527-560` — three cases, including that it is off in `composite` and that it bypasses the `onWillMove` veto |
| `data-snav-scroll="center"` | `src/spatial/spatial.browser.test.ts:562-596` — centres when the container asks, stays at `nearest` when it does not |
| `explainMove` parity with the real winner | `src/debug.browser.test.ts:56-111` and `:113-180` — see the reversal below |
| The spatial plugin's handling of `scrollX` | `src/spatial/spatial.browser.test.ts:660-726` — five cases: both signs, a zero value, the 24-pixel rate at full and half deflection, and silence when scrolling is off |
| Zero-size filter with one zero dimension | `src/spatial/spatial.browser.test.ts:598-646` — three cases pinning the `||` filter [ADR-0009](0009-hidden-candidates.md) C1 shipped in this pull request: no size at all (`:613`), flat on a single axis (`:622`), and a 1px hairline kept (`:636`), which is the bound that stops the rule reaching a real target |
| `WeakRef` fallback path | `src/spatial/spatial.browser.test.ts:809-841` — the property is deleted from `globalThis` for the duration of the case, so the strong-reference branch actually runs |
| The inherited hard limits | depth 16 at `src/spatial/spatial.browser.test.ts:788-807`; `MAX_PADS = 4` and `MAX_BUTTONS = 20` at `src/gamepad/gamepad.browser.test.ts:303-325`, driven by a harness offering six slots and a settable button count so the engine's own bound is what the assertion reads |
...
[ADR-0008](0008-shadow-dom.md) (`src/spatial/spatial.browser.test.ts:870-892`), which ships skipped
...
- Suite counts, this repository, 2026-09-21: `bun run test:unit` → 100 passed in 10 files;
  ...
  `src/spatial/spatial.browser.test.ts:871` ([ADR-0008](0008-shadow-dom.md)).
