...
| [0022](0022-virtual-keyboard.md) | The virtual keyboard — layout data, insertion, and what closes it | Proposed, two riders | Keys take real focus; a layout is data in its own module with no registry; insertion is `beforeinput`, mutate, `input`, with the framework-tracking risk named; composition deferred to its own ADR. Open: (O1) whether `back` keeps or reverts the text, (O2) whether the field keeps a caret. Nothing built yet. |
