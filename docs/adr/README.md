...
1. Take the next free number. The highest in use is ADR-0030, so the next is
   ADR-0031. Numbers are never reused, and a superseded record keeps its number.
...
| [0017](0017-size-budgets.md) | Size budgets: measure before capping | Accepted | Two measured lines per subpath, caps written only after a first measurement here, and a line without a cap fails the run. Eleven lines are measured and capped as of 2026-09-21; a twelfth, the vue adapter's, on 2026-09-23; a thirteenth, the svelte adapter's, and a fourteenth, the angular adapter's, on 2026-09-24. The core cap went to 3.50 kB the same day, before the static filter of [ADR-0030](0030-refused-focus-next-candidate.md). |
