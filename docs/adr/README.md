...
- Length: 60 to 150 lines is the target **for a new record**. The twenty-six below
  run from 96 to 638 lines (`wc -l docs/adr/0*.md`, 2026-09-23), because an
...
1. Take the next free number. The highest in use is ADR-0027, so the next is
   ADR-0028. Numbers are never reused, and a superseded record keeps its number.
...
| [0011](0011-package-layout-and-adapters.md) | One package, subpath exports, adapters as subpaths | Accepted | One published package with twelve subpaths built today, adapters as optional peers, React and Vue shipped and passing the parity suite, zero runtime dependencies enforced by `check:package`. Its one rider closed on 2026-09-22: the vanilla auto-mount helper is `/auto` ([ADR-0023](0023-vanilla-auto-mount.md)). |
...
| [0017](0017-size-budgets.md) | Size budgets: measure before capping | Accepted | Two measured lines per subpath, caps written only after a first measurement here, and a line without a cap fails the run. Eleven lines are measured and capped as of 2026-09-21; a twelfth, the vue adapter's, on 2026-09-23. |
