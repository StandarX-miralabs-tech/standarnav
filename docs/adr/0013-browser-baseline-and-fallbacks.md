...
- A CI check must still be added that greps the built `dist` for `WeakRef` and for `.at(` and fails
  ...
  (`.github/workflows/ci.yml`) now runs eight jobs — lint (`:18`), typecheck (`:32`), build (`:42`),
  ...
  vue-floor (`:136-161`, the same for Vue) and svelte-floor (`:169-193`, the same for Svelte, with the
  unit suite too), which is ten checks — and the build job runs `bun run build`, the `git diff --exit-code` exports-map drift
