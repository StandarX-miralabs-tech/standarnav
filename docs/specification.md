...
- **R34.** Delivery order, the owner's decision of 2026-09-18, recorded in
  ...
  Vue, Svelte, Angular. React and, since 2026-09-23, Vue exist
  ([ADR-0027](adr/0027-vue-adapter.md)); Svelte and Angular are not written.
- **R35.** Every adapter is a subpath export (`@standarx/nav/react`, `/vue`, `/svelte`, `/angular`)
  ...
  `./react` and `./vue` are published (`package.json` exports), with `react` and
  `react-dom` at `>=18.3.0` and `vue` at `>=3.3.0` as optional peers, each floor run by its own CI
  job. Neither imports the engines — `src/react/react.tsx` imports the
  ...
  four and `vue` — so the consumer passes
...
- **R36.** No adapter ships until it passes the shared suite of `src/adapter-parity.ts`, the same one
  ...
  `src/vue/vue.browser.test.ts:921`; `bun run test:browser` on the three engines, 2026-09-23).
