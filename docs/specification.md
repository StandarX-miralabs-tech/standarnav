...
- **R34.** Delivery order, the owner's decision of 2026-09-18, recorded in
  ...
  Vue, Svelte, Angular. React, since 2026-09-23 Vue and since 2026-09-24 Svelte and Angular exist
  ([ADR-0027](adr/0027-vue-adapter.md), [ADR-0028](adr/0028-svelte-adapter.md),
...
- **R35.** Every adapter is a subpath export (`@standarx/nav/react`, `/vue`, `/svelte`, `/angular`)
  ...
  `./react`, `./vue`, `./svelte` and `./angular` are published (`package.json` exports), with
  `react` and `react-dom` at `>=18.3.0`, `vue` at `>=3.3.0`, `svelte` at `>=5.0.0` and
  `@angular/core` at `>=20.0.0` as optional peers, each floor run by its own CI job. None imports
  the engines — `src/react/react.tsx` imports the
  ...
  four and `vue`, `src/svelte/svelte.ts` the same four and `svelte`, and
  `src/angular/angular.ts` the same four and `@angular/core` — so the consumer passes
...
- **R36.** No adapter ships until it passes the shared suite of `src/adapter-parity.ts`, the same one
  ...
  so do Svelte and Angular (`src/svelte/svelte.browser.test.ts:724` and
  `src/angular/angular.browser.test.ts:1143`; the same command on the three engines, 2026-09-24).
